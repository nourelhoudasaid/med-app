...
router.post('/register', upload.fields([
  ...
  const { name, email, password, role, specialization, availability, medicalHistory } = req.body;
  ...
  try {
    ...
    const newUser = new User({
      ...
      diplomaImage: role === 'Doctor' && req.files['diplomaImage'] ? req.files['diplomaImage'][0].path : null,
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (user.role === 'Doctor' && !user.isValidated) {
      return res.status(403).json({ message: 'Your account has not been validated yet. Please wait for admin approval.' });
    }
